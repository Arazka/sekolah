              ...
              <img src="{{ asset('storage/'.$profil->first()->foto) }}" class="absolute block object-cover w-full h-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="..." />
                <div class="absolute top-0 -left-0 w-full h-full bg-black bg-opacity-40">
                    <div class="mx-auto max-w-screen-xl h-full flex flex-col justify-center items-center p-2">
                        <div data-aos="fade-up" data-aos-delay="100" data-aos-duration="500" class="text-center text-3xl sm:text-5xl font-bold text-white mb-8 tracking-normal">Guru Mata Pelajaran</div>
                        <nav class="flex" aria-label="Breadcrumb">
                            ...
                            <li data-aos="fade-up" data-aos-delay="100" data-aos-duration="500" aria-current="page">
                                ...
                                <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 dark:text-gray-400">Guru Mata Pelajaran</span>
                                </div>
                            </li>
                            </ol>
                        </nav>
    ...
    <div class="mx-auto max-w-screen-xl py-16 px-2">
        <div class="mx-auto max-w-screen-xss sm:max-w-screen-sm md:max-w-screen-md xl:max-w-screen-xl">
                <div class="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 gap-8">
                    ...
                    <div class="max-w-[30rem] bg-white shadow-lg group">
                        <div class="block relative overflow-hidden transform">
                            <img class="w-full h-auto transition-transform transform group-hover:scale-105 duration-500" src="{{ asset('storage/'.$mapel->foto) }}" alt="{{ $mapel->nama }}" />
                        </div>
                        <div class="p-5 text-center">
                            <div>
                                <h5 class="mb-2 text-2xl font-semibold tracking-wide text-gray-900">{{ $mapel->nama }}</h5>
                            </div>
                            <p class="my-2 uppercase font-medium tracking-widest text-gray-400 text-sm">{{ $mapel->nama_mapel }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="justify-center flex mt-16">
                  {{ $mapels->links('pagination.pagination-1') }}
                </div>
                @endif
        </div>
    </div>
