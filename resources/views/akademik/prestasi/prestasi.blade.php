        ...
        <div class="mx-auto max-w-screen-xss sm:max-w-screen-sm md:max-w-screen-md xl:max-w-screen-xl">
                <div class="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 gap-8">
                    ...
                    <div class="max-w-md bg-white shadow-lg group">
                        <a href="{{ url('prestasi/'.$item->nama) }}" class="block relative overflow-hidden transform">
                        ...
                        <div class="p-5">
                            <a href="{{ url('prestasi/'.$item->nama) }}">
                                <h5 class="mb-2 text-2xl font-medium tracking-tight text-gray-900 group-hover:text-blue-700 transition duration-500">{{ Str::limit($item->nama, 45) }}</h5>
                            </a>
                            <p class="my-4 font-normal text-gray-700 dark:text-gray-400">{{ Str::limit(strip_tags($item->deskripsi), 120) }}</p>
                            <a href="{{ url('prestasi/'.$item->nama) }}" class="inline-flex items-   center font-medium text-blue-700 hover:underline"> Read more </a>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="justify-center flex mt-16">
                    {{ $prestasi->links('pagination.pagination-1') }}
                </div>
                @endif
        </div>
    </div>
