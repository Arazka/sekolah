            ...
            <img src="{{ asset('storage/'.$profil->foto) }}" class="absolute block object-cover w-full h-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="..." />
                <div class="absolute top-0 -left-0 w-full h-full bg-black bg-opacity-40">
                    <div class="mx-auto max-w-screen-xl h-full flex flex-col justify-center items-center p-2">
                        <div data-aos="fade-up" data-aos-delay="100" data-aos-duration="500" class="text-center text-3xl sm:text-5xl font-bold text-white mb-8 tracking-normal">Detail Ekstrakurikuler</div>
                        <nav class="flex" aria-label="Breadcrumb">
                            ...
                            <li data-aos="fade-up" data-aos-delay="100" data-aos-duration="500" aria-current="page">
                                ...
                                <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 dark:text-gray-400">Detail Ekstrakurikuler</span>
                                </div>
                            </li>
                            </ol>
                        </nav>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 md:gap-14">
                    <div id="detail-prestasi" class="mb-8 md:mb-0 md:col-span-2">
                        <div class="w-full max-w-full md:h-96 h-auto overflow-hidden rounded-md">
                            <img class="object-cover w-full h-full" src="{{ asset('storage/'.$detail->foto) }}" alt="{{ $detail->nama }}" />
                        </div>                     
                        {{-- <img class="h-auto max-w-full rounded-md object-cover" src="{{ asset('storage/'.$detail->foto) }}" alt="{{ $detail->nama }}"/> --}}
                        <div class="my-4 xss:my-6">
                            <h5 class="text-2xl md:text-4xl font-medium tracking-tight text-gray-900">{{ $detail->nama }}</h5>
                        </div>
                        ...
                        <div class="w-full text-left bg-slate-100 p-8 rounded-md sm:bg-white sm:p-0 sm:rounded-none">
                            <div class="flex items-center mb-4 flex-col sm:mb-0 sm:flex-row">
                                <span class="font-semibold sm:w-40">Kategori </span>
                                <span class="font-normal capitalize text-gray-700 text-center"><span class="hidden sm:inline">:</span> {{ $detail->kategori }}</span>
                            </div>
                            <div class="flex items-center mb-4 flex-col sm:mb-0 sm:flex-row">
                                <span class="font-semibold sm:w-40">Jadwal Pertemuan </span>
                                <span class="font-normal capitalize text-gray-700 text-center"><span class="hidden sm:inline">:</span> {{ $detail->jadwal_pertemuan }}</span>
                            </div>
                            <div class="flex items-center mb-4 flex-col sm:mb-0 sm:flex-row">
                                <span class="font-semibold sm:w-40">Koordinator Kegiatan </span>
                                <span class="font-normal capitalize text-gray-700 text-center"><span class="hidden sm:inline">:</span> {{ $detail->koordinator_kegiatan }}</span>
                            </div>
                            <div class="flex items-center flex-col sm:mb-0 sm:flex-row">
                                <span class="font-semibold sm:w-40">Lokasi Kegiatan </span>
                                <span class="font-normal capitalize text-gray-700 text-center"><span class="hidden sm:inline">:</span> {{ $detail->lokasi_kegiatan }}</span>
                            </div>
                        </div>
                    </div>
                    <div id="another-ekstrakurikuler" class="w-full">
                        <div class="">
                            <div class="text-2xl text-left font-medium"><span class="text-blue-700">Ekstrakurikuler</span> Lainnya</div>
                            <hr class="h-px mt-2 bg-gray-200 border-0 dark:bg-gray-700">
                            @if ($ekstra->count() > 0)
                            @foreach ($ekstra as $item)
                            <a href="{{ url('ekstrakurikuler/'.$item->nama) }}" class="flex items-center group w-full mt-6 rounded-md overflow-hidden hover:shadow-lg transition-transform transform hover:scale-105">
